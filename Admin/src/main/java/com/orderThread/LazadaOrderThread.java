...
public class LazadaOrderThread implements Runnable {
    ...
    public void run() {
        ...
        if (rt.getObj() != null) {
            ...
            if ("1".equals(lazadaShopnumber) == true || "2".equals(lazadaShopnumber) == true) {
                calcuOrderDiff = 1;
            }
           /* if ("1".equals(this.multiFlag) == true) {// 如果配置文件中要求启用多仓规则,那么去获取
                // 只要有店铺需要下载订单,则先去获取多仓sku列表
                RetCode rtx = lazadaMgr.findManyStorageSkuFlag();// 查询用户是否有使用多仓,即查看DB_MULTI_WAREHOUSE表里有无记录
                if (rtx.getCode() == 0) {
